...
if ([int]$engine_count -ne 0) {
    for ($i = 1; $i -le [int]$engine_count; $i++) {
        ...
        $composeLines += "      - POSTGRES_DB_PORT=`${DOCKER_DB_PORT:-5432}"
        $composeLines += "      - POSTGRES_SSL_MODE=`${DOCKER_POSTGRES_SSL_MODE:-disable}"
        $composeLines += '      # Mail source metadata only. Authentication material is resolved from the credential reference.'
        $composeLines += "      - CROWLER_MAIL_CONNECTOR_HOST=`${CROWLER_MAIL_CONNECTOR_HOST:-}"
        $composeLines += "      - CROWLER_MAIL_CONNECTOR_PORT=`${CROWLER_MAIL_CONNECTOR_PORT:-}"
        $composeLines += "      - CROWLER_MAIL_CONNECTOR_USERNAME=`${CROWLER_MAIL_CONNECTOR_USERNAME:-}"
        $composeLines += "      - CROWLER_MAIL_CREDENTIAL_REF=`${CROWLER_MAIL_CREDENTIAL_REF:-}"
        $composeLines += "      - CROWLER_MAIL_PROVIDER_ACCOUNT_ID=`${CROWLER_MAIL_PROVIDER_ACCOUNT_ID:-}"
        $composeLines += "      - CROWLER_MAIL_PROVIDER_PROJECT_ID=`${CROWLER_MAIL_PROVIDER_PROJECT_ID:-}"
        $composeLines += "      - CROWLER_MAIL_PROVIDER_TENANT_ID=`${CROWLER_MAIL_PROVIDER_TENANT_ID:-}"
        $composeLines += "      - CROWLER_MAIL_PROVIDER_CLIENT_ID=`${CROWLER_MAIL_PROVIDER_CLIENT_ID:-}"
        $composeLines += "      - CROWLER_MAIL_PROVIDER_SUBSCRIPTION_ID=`${CROWLER_MAIL_PROVIDER_SUBSCRIPTION_ID:-}"
        $composeLines += "      - CROWLER_MAIL_LISTENER_ENABLED=`${CROWLER_MAIL_LISTENER_ENABLED:-false}"
        $composeLines += "      - CROWLER_MAIL_LISTENER_BUFFER_SIZE=`${CROWLER_MAIL_LISTENER_BUFFER_SIZE:-128}"
        $composeLines += "      - CROWLER_MAIL_LISTENER_COALESCE_WINDOW=`${CROWLER_MAIL_LISTENER_COALESCE_WINDOW:-1s}"
        $composeLines += "      - CROWLER_MAIL_LISTENER_RECONNECT_BACKOFF=`${CROWLER_MAIL_LISTENER_RECONNECT_BACKOFF:-5s}"
        $composeLines += "      - CROWLER_MAIL_LISTENER_MAX_RECONNECT_BACKOFF=`${CROWLER_MAIL_LISTENER_MAX_RECONNECT_BACKOFF:-1m}"
        $composeLines += "      - CROWLER_MAIL_LISTENER_IDLE_REISSUE_INTERVAL=`${CROWLER_MAIL_LISTENER_IDLE_REISSUE_INTERVAL:-25m}"
        $composeLines += "      - TZ=`${VDI_TZ:-UTC}"
        $composeLines += "    build:"
        $composeLines += "      context: ."

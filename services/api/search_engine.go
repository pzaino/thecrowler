...
func performSearch(query string, db *cdb.Handler) (SearchResult, error) {
	...
	if config.API.ReturnContent {
		queryBody = `
		SELECT DISTINCT
			si.title, si.page_url, si.summary, wo.object_content, si.detected_type, si.detected_language, '' AS content
		FROM
			SearchIndex si
		LEFT JOIN
		...
		queryBody = `
		SELECT DISTINCT
			si.title, si.page_url, si.summary, si.detected_type, si.detected_language, '' AS content
		FROM
			SearchIndex si
		LEFT JOIN
			WebObjectsIndex woi ON si.index_id = woi.index_id
